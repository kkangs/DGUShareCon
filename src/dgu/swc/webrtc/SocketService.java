...
public class SocketService {
	 ...
	 * @param base64Image 전송할 base64 이미지
	 */
	public void imageUpload(String email, String base64EncodedBitmap) {
	
		try {
		JSONObject data = new JSONObject();
		data.put("email", email);
		data.put("image", base64EncodedBitmap);
		...
		eventHandler.obtainMessage(SocketEvent.MSG_IMAGE_UPLOAD, SocketEvent.FAILURE, 0, e.getMessage()).sendToTarget();
		}
	}
	/**
	 * GCM RegId 업데이트 추가
	...
	public void regIdUpload(String email, String regId)
	{
		try {
		JSONObject data = new JSONObject();
		data.put("email", email);
		data.put("regid", regId);
			...
			socket.emit("regIdUpload", data);
		} catch(JSONException e) {
			eventHandler.obtainMessage(SocketEvent.MSG_REGID_UPLOAD, SocketEvent.FAILURE, 0, e.getMessage()).sendToTarget();
		}
	}
